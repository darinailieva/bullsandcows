...
public interface GameRepository extends JpaRepository<Game, Integer> {
  ...
  @Query(value = "select g.username as username, "
    + "count(g.finished) as numberOfFinishedGames, "
    + "min(g.times_played) as bestTimesPlayed, "
    + " min(extract(minute from (g.last_modified_date - g.created_date))) as bestTimeInMinutes"
    + " from games g where g.finished=true "
    + "group by g.username order by min(g.times_played)",
    nativeQuery = true)
